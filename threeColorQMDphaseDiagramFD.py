...
def solution2(T, mu):
    ode_order = 5
    nsteps = 100000
    ode_integrator = 'dop853' #'vode'#'lsoda'
    ode_method = 'bdf'#'adams'
    r_tol = 1e-15
    a_tol = 1e-15
    ode15s = ode(f)
    ode15s.set_integrator(ode_integrator, nsteps = nsteps, rtol=r_tol, atol=a_tol)
...
start = time.time()
T_min, T_max = 10, 60#15, 40
mu_min, mu_max = 250, 350#250, 450
N_T, N_mu = 20, 20#8, 20
